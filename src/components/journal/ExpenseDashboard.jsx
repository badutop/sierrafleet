...
// Le cadre (border-sidebar, ajouté sur le conteneur rendu plus bas) est
// commun à toutes les cartes — seul le fond/texte reste teinté par type de
// frais, pour rester assorti au menu latéral.
const TYPE_CONFIG = {
  carburant:      { label: "Carburant",       color: "bg-blue-500/15 text-blue-700",    icon: Fuel },
  peage:          { label: "Péage",            color: "bg-purple-500/15 text-purple-700", icon: BarChart3 },
  rations:        { label: "Rations",          color: "bg-emerald-500/15 text-emerald-700", icon: Utensils },
  contravention:  { label: "Contraventions",   color: "bg-destructive/15 text-destructive", icon: AlertTriangle },
  transport:      { label: "Transport",        color: "bg-amber-500/15 text-amber-700",  icon: Truck },
  entretien:      { label: "Entretien garage", color: "bg-cyan-500/15 text-cyan-700",     icon: Wrench },
  pneus:          { label: "Pneus",            color: "bg-slate-500/15 text-slate-700",  icon: Disc },
  achat_pieces:   { label: "Achat pièces",     color: "bg-indigo-500/15 text-indigo-700", icon: ShoppingCart },
  autre:          { label: "Autre",            color: "bg-muted text-muted-foreground",        icon: BarChart3 },
};
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {Object.entries(TYPE_CONFIG).map(([key, cfg]) => {
          ...
          return (
            <div key={key} className={`rounded-xl border border-sidebar px-4 py-3 flex flex-col gap-1 ${cfg.color}`}>
