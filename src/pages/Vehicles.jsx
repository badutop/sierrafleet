...
import { Plus, Search, Truck, Gauge, Calendar } from "lucide-react";
...
export default function Vehicles() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["vehicles"] }); setDialogOpen(false); setForm({}); toast.success("Véhicule ajouté"); },
  });
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              {[["immatriculation","Immatriculation"],["marque","Marque"],["modele","Modèle"],["annee","Année"],["couleur","Couleur"],["km_actuel","Km actuel"]].map(([key,label]) => (
                <div key={key}>
                  <Label className="text-xs">{label}</Label>
                  <Input className="mt-1" value={form[key] || ""} onChange={e => setForm({...form, [key]: key === "annee" || key === "km_actuel" ? Number(e.target.value) : e.target.value})} />
                </div>
              ))}
              <div>
              ...
              <div>
                ...
                <Input className="mt-1" type="number" value={form.capacite_charge_tonnes || ""} onChange={e => setForm({...form, capacite_charge_tonnes: Number(e.target.value)})} />
              </div>
            </div>
