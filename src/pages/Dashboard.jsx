...
function StatCard({ title, value, subtitle, icon: Icon, color, trend, trendLabel, className }) {
  ...
  return (
    <div className={`${c.card} rounded-xl border border-sidebar p-5 hover:shadow-lg transition-shadow group ${className || ""}`}>
      ...
      <p className={`text-xs mt-1 ${c.text} opacity-80`}>{title}</p>
      {subtitle && <p className={`text-xs mt-0.5 ${c.text} opacity-60`}>{subtitle}</p>}
      {trendLabel && <p className={`text-[11px] mt-1 ${c.text} opacity-50`}>{trendLabel}</p>}
    </div>
  );
}
